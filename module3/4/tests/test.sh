...
SHARED_MEMORY_KEY="0x4d340001"
SEMAPHORE_KEY="0x4d340002"
...
cleanup_ipc()
{
    ipcrm -M "$SHARED_MEMORY_KEY" >/dev/null 2>&1 || true
    ipcrm -S "$SEMAPHORE_KEY" >/dev/null 2>&1 || true
}
...
ipc_resources_exist()
{
    ipcs -m | grep -qi "$SHARED_MEMORY_KEY" \
        || ipcs -s | grep -qi "$SEMAPHORE_KEY"
}

wait_for_producer()
{
    for _ in $(seq 1 100); do
        if ipcs -m | grep -qi "$SHARED_MEMORY_KEY" \
            && ipcs -s | grep -qi "$SEMAPHORE_KEY"; then
            return 0
        fi
...
# Перед тестами удаляем только старые объекты с ключами этой лабораторной.
cleanup_ipc
